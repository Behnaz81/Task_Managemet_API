...
from users.serializers import (RegisterSerializer, 
                               ...
                               DeleteTeamMembershipSerializer, 
                               ReadTeamMembershipSerializer)
...
class SetManager(APIView):
    ...
    def post(self, request, user_id):
        ...
        user.role = 'manager'
        user.save()
        return Response({'details': 'successfully set to manager'}, 
                        status=status.HTTP_200_OK)
...
class SetTeamLeader(APIView):
    ...
    def post(self, request, user_id):
        ...
        user.save()
        return Response({'details': 'successfully set to team leader'}, 
                        status=status.HTTP_200_OK)
...
class CreateTeam(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response({'details':'team created successfully', 
...
class ListTeam(APIView):
    ...
    def get(self, request):
        ...
        return Response({'details':'data retrieved successfully', 
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user_to_add.role == 'manager':
                return Response({'details': "You can't add managers to teams"}, 
                                status=status.HTTP_400_BAD_REQUEST)

            if TeamMembership.objects.filter(user=user_to_add).exists():
                return Response({'details': 'This user already has a team'}, 
                                status=status.HTTP_400_BAD_REQUEST)
            
            if user_to_add.role == 'teamleader':
                if TeamMembership.objects.filter(team=team, user__role='teamleader').exists():
                    return Response({'details': 'The team already has a team leader'}, 
                                    status=status.HTTP_400_BAD_REQUEST)
...
class DeleteTeam(APIView):
    ...
    def delete(self, request, id):
        ...
        try:
            ...
            return Response({'details': "You are not a member of this team"}, 
                            status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response({'details': "This member isn't in this team"}, 
